...
class CNP(torch.nn.Module):  ### upsampling is in block_in
    def __init__(self, in_channels=1, channels=12, kernel_size=3, block_layers=2, outstage=8, instage=1):
        ...
        self.block_in = self.make_block(in_channels=in_channels, channels=channels, out_channels=channels, kernel_size=kernel_size, block_layers=block_layers)
        ...
        self.inner_blocks = nn.ModuleList(
            [
                nn.ModuleList([
                    self.make_block(in_channels=_channels[out_idx], channels=channels, out_channels=channels, kernel_size=3, block_layers=1) 
        ...
        self.prune_blocks = nn.ModuleList(
            [
                nn.ModuleList([
                    ConvWithPrune(in_channels=channels, out_channels=channels,kernel_size=3, stride=1, bias=True, dimension=3)
                for in_idx in range(instage)])
            for out_idx in range(outstage)]
        )
    ...
    def forward(self, x_low, x_high_args):
        ...
        for out_idx in range(self.outstage):
            ...
            # tmp_occ = sort_sparse_tensor(insiblings)
            # assert (tmp_occ.C == x_occ_lst[out_idx].C).all()
            # assert (tmp_occ.F == x_occ_lst[out_idx].F).all()
    ...
    def encode(self, x_low, x_high_args, DBG=True):
        ...
        # dec_occ_lst = self.decode({'enc_bytes': bitstream, 'x_low': x_low})
        # for dec_occ, ground_truth in zip(dec_occ_lst, out_set['ground_truth_list']):
            # assert (dec_occ == ground_truth).all()
    ...
    def decode(self, enc_out):
        ...
        for out_idx in range(self.outstage):
            ...
            for in_idx in range(self.instage):
                ...
                coord, out_cls = self.basic_module(insiblings, out_glob, out_idx, in_idx)
                occupancy = BAC.decode(out_cls.view(-1,1).detach().cpu(), bitstream).unsqueeze(1)
                ...
                out_cls_list.append(out_cls)
            x_occ = sort_sparse_tensor(insiblings)
            dec_occ_list.append(x_occ.F)
            if out_idx == self.outstage-1:
                break
            if outsiblings is None:
                outsiblings = x_occ
            else:
                outsiblings = merge_two_frames(outsiblings, x_occ)
            out_glob_new = self.outter_blocks[out_idx](outsiblings)
            out_glob_new = ME.SparseTensor(out_glob_new.F, coordinate_map_key=x_glob.coordinate_map_key, coordinate_manager=x_glob.coordinate_manager, device=x_glob.device)
            glob_new = x_glob + out_glob_new
            out_glob_split = self.mask_data(glob_new, mask_list=mask_lst)
